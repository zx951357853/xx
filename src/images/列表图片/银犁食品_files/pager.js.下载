...
var pagination = {
	...
	click: function (n) {
		//这里自己实现
		//这里可以用this或者pagination访问pagination对象
		//return false;
	},
	...
	gopage: function () {
		...
		if (isNaN(str_page)) {
			$('#' + this.gopageTextboxId).val(this.pno);
			return;
		}
	...
	generPageHtml: function (config, enforceInit) {
		...
		if (this.isShowFirstPageBtn) {
			if (this.hasPrv) {
				str_first = '<a ' + this._getHandlerStr(1) + ' class="page" title="'
					+ (this.lang.firstPageTipText || this.lang.firstPageText) + '">' + this.lang.firstPageText + '</a>';
			} else {
				str_first = '<span class="page disabled">' + this.lang.firstPageText + '</span>';
			}
		}
		if (this.isShowPrePageBtn) {
			if (this.hasPrv) {
				str_prv = '<a ' + this._getHandlerStr(this.prv) + ' class="pageSwitch" title="'
					+ (this.lang.prePageTipText || this.lang.prePageText) + '">' + this.lang.prePageText + '</a>';
			} else {
				str_prv = '<span class="pageSwitch disabled">' + this.lang.prePageText + '</span>';
			}
		}
		if (this.isShowNextPageBtn) {
			if (this.hasNext) {
				str_next = '<a ' + this._getHandlerStr(this.next) + ' class="pageSwitch"  title="'
					+ (this.lang.nextPageTipText || this.lang.nextPageText) + '">' + this.lang.nextPageText + '</a>';
			} else {
				str_next = '<span class="pageSwitch disabled">' + this.lang.nextPageText + '</span>';
			}
		}
		if (this.isShowLastPageBtn) {
			if (this.hasNext) {
				str_last = '<a ' + this._getHandlerStr(this.total) + ' class="page" title="'
					+ (this.lang.lastPageTipText || this.lang.lastPageText) + '">' + this.lang.lastPageText + '</a>';
			} else {
				str_last = '<span class="page disabled">' + this.lang.lastPageText + '</span>';
			}
		}
		var str = '';
		...
		if (this.isGoPage) {
			gopage_info = '<span class="goPageBox">' + this.lang.gopageBeforeText + '<span id="' + this.gopageWrapId + '">'+
				'<input type="text" id="' + this.gopageTextboxId + '"  onkeypress="return pagination.keypress_gopage(event);" value="' + this.pno + '" /></span>' + this.lang.gopageAfterText
				+ '<input type="button" id="' + this.gopageButtonId + '" onclick="pagination.gopage()" value="'
		...
		if (this.total <= 8) {
			...
			if (this.pno <= 5) {
				...
				str += '<a ' + this._getHandlerStr(this.total) + ' title="'
					+ this.lang.buttonTipBeforeText + '2' + this.lang.buttonTipAfterText + '">'+ this.total +'</a>';
			} else {
	...
	init: function (config) {
		...
		if (this.total < 3) {
			this.isGoPage = false;
			this.isShowLastPageBtn = false;
			this.isShowFirstPageBtn = false;
		}
	},
